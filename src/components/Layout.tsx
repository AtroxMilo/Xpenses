...
export function Layout() {
  ...
  const onAddScreen = pathname === '/add' || pathname.startsWith('/edit/')
    ...
    <div className="mx-auto flex min-h-full max-w-lg flex-col bg-slate-100 dark:bg-slate-950">
      ...
      {!onAddScreen && (
        <button
          ...
          className="fixed bottom-24 left-1/2 z-20 -translate-x-1/2 rounded-full bg-slate-900 px-6 py-3 text-base font-semibold text-white shadow-lg shadow-slate-900/30 active:scale-95 dark:bg-white dark:text-slate-900"
        >
          + Add expense
        </button>
      )}
